...
/* Department / Employee */
typedef struct { ason_string_t title; } Department;
ASON_FIELDS(Department, 1,
    ASON_FIELD(Department, title, "title", str))
...
ASON_FIELDS(Employee, 5,
    ASON_FIELD(Employee, id,     "id",     i64),
...
ASON_FIELDS(WithMap, 2,
    ASON_FIELD(WithMap, name,  "name",  str),
    ASON_FIELD(WithMap, attrs, "attrs", map_si))

/* Address / Nested */
typedef struct { ason_string_t city; int64_t zip; } Address;
ASON_FIELDS(Address, 2,
    ASON_FIELD(Address, city, "city", str),
    ASON_FIELD(Address, zip,  "zip",  i64))

typedef struct { ason_string_t name; Address addr; } Nested;
ASON_FIELDS(Nested, 2,
    ASON_FIELD(Nested, name, "name", str),
...
ASON_FIELDS(AllTypes, 17,
    ASON_FIELD(AllTypes, b,          "b",          bool),
...
ASON_FIELDS(Building, 4,
    ASON_FIELD(Building, name,        "name",        str),
...
ASON_FIELDS(Street, 3,
    ASON_FIELD(Street, name,      "name",      str),
...
ASON_FIELDS(District, 3,
    ASON_FIELD(District, name,       "name",       str),
...
ASON_FIELDS(Region, 2,
    ASON_FIELD(Region, name,   "name",   str),
...
ASON_FIELDS(Country, 5,
    ASON_FIELD(Country, name,         "name",         str),
...
/* 7-level: Universe > Galaxy > SolarSystem > Planet > Continent > Nation > State */
typedef struct { ason_string_t name; ason_string_t capital; int64_t population; } State;
ASON_FIELDS(State, 3,
    ASON_FIELD(State, name,       "name",       str),
    ASON_FIELD(State, capital,    "capital",    str),
    ASON_FIELD(State, population, "population", i64))
ASON_VEC_STRUCT_DEFINE(State)

typedef struct { ason_string_t name; ason_vec_State states; } Nation;
ASON_FIELDS(Nation, 2,
    ASON_FIELD(Nation, name,   "name",   str),
    ASON_FIELD_VEC_STRUCT(Nation, states, "states", State))
ASON_VEC_STRUCT_DEFINE(Nation)

typedef struct { ason_string_t name; ason_vec_Nation nations; } Continent;
ASON_FIELDS(Continent, 2,
    ASON_FIELD(Continent, name,    "name",    str),
...
ASON_FIELDS(Planet, 4,
    ASON_FIELD(Planet, name,       "name",       str),
...
typedef struct { ason_string_t name; ason_string_t star_type; ason_vec_Planet planets; } SolarSystem;
ASON_FIELDS(SolarSystem, 3,
    ASON_FIELD(SolarSystem, name,      "name",      str),
...
typedef struct { ason_string_t name; double star_count_billions; ason_vec_SolarSystem systems; } Galaxy;
ASON_FIELDS(Galaxy, 3,
    ASON_FIELD(Galaxy, name,                "name",                str),
...
typedef struct { ason_string_t name; double age_billion_years; ason_vec_Galaxy galaxies; } Universe;
ASON_FIELDS(Universe, 3,
    ASON_FIELD(Universe, name,              "name",              str),
    ASON_FIELD(Universe, age_billion_years,  "age_billion_years", f64),
    ASON_FIELD_VEC_STRUCT(Universe, galaxies, "galaxies", Galaxy))

/* ServiceConfig */
typedef struct { ason_string_t host; int64_t port; int64_t max_connections; bool ssl; double timeout_ms; } DbConfig;
ASON_FIELDS(DbConfig, 5,
    ASON_FIELD(DbConfig, host,            "host",            str),
...
typedef struct { bool enabled; int64_t ttl_seconds; int64_t max_size_mb; } CacheConfig;
ASON_FIELDS(CacheConfig, 3,
    ASON_FIELD(CacheConfig, enabled,      "enabled",      bool),
    ASON_FIELD(CacheConfig, ttl_seconds,  "ttl_seconds",  i64),
    ASON_FIELD(CacheConfig, max_size_mb,  "max_size_mb",  i64))

typedef struct { ason_string_t level; ason_opt_str file; bool rotate; } LogConfig;
ASON_FIELDS(LogConfig, 3,
    ASON_FIELD(LogConfig, level,  "level",  str),
...
ASON_FIELDS(ServiceConfig, 7,
    ASON_FIELD(ServiceConfig, name,     "name",     str),
    ASON_FIELD(ServiceConfig, version,  "version",  str),
    ASON_FIELD_STRUCT(ServiceConfig, db,    "db",    &DbConfig_ason_desc),
    ASON_FIELD_STRUCT(ServiceConfig, cache, "cache", &CacheConfig_ason_desc),
...
typedef struct { ason_string_t text; } Note;
ASON_FIELDS(Note, 1, ASON_FIELD(Note, text, "text", str))

typedef struct { int64_t id; double value; ason_string_t label; } Measurement;
ASON_FIELDS(Measurement, 3,
    ASON_FIELD(Measurement, id,    "id",    i64),
    ASON_FIELD(Measurement, value, "value", f64),
    ASON_FIELD(Measurement, label, "label", str))

typedef struct { int64_t a; double b; int64_t c; } Nums;
ASON_FIELDS(Nums, 3,
    ASON_FIELD(Nums, a, "a", i64),
    ASON_FIELD(Nums, b, "b", f64),
    ASON_FIELD(Nums, c, "c", i64))

typedef struct { ason_string_t val; } Special;
ASON_FIELDS(Special, 1, ASON_FIELD(Special, val, "val", str))

typedef struct { ason_vec_vec_i64 data; } Matrix3D;
ASON_FIELDS(Matrix3D, 1, ASON_FIELD(Matrix3D, data, "data", vec_vec_i64))

typedef struct { ason_vec_i64 items; } WithVec;
...
int main(void) {
    ...
    {
        const char* input = "{id,name,dept:{title},skills,active}:(1,Alice,(Manager),[rust],true)";
        Employee emp = {0};
        ason_err_t err = ason_load_Employee(input, strlen(input), &emp);
        assert(err == ASON_OK);
        printf("   id=%lld name=%s dept=%s skills=[", (long long)emp.id, emp.name.data, emp.dept.title.data);
        for (size_t i = 0; i < emp.skills.len; i++) { if (i) printf(","); printf("%s", emp.skills.data[i].data); }
        printf("] active=%s\n\n", emp.active ? "true" : "false");
        ason_string_free(&emp.name); ason_string_free(&emp.dept.title);
        for (size_t i = 0; i < emp.skills.len; i++) ason_string_free(&emp.skills.data[i]);
    ...
    {
        const char* input = "{id:int,name:str,dept:{title:str},skills:[str],active:bool}:"
            "(1,Alice,(Manager),[Rust,Go],true),"
            "(2,Bob,(Engineer),[Python],false),"
            "(3,\"Carol Smith\",(Director),[Leadership,Strategy],true)";
        Employee* emps = NULL; size_t cnt = 0;
        ason_err_t err = ason_load_vec_Employee(input, strlen(input), &emps, &cnt);
        assert(err == ASON_OK);
        for (size_t i = 0; i < cnt; i++) {
            printf("   id=%lld name=%s dept=%s\n", (long long)emps[i].id, emps[i].name.data, emps[i].dept.title.data);
            ason_string_free(&emps[i].name); ason_string_free(&emps[i].dept.title);
            for (size_t j = 0; j < emps[i].skills.len; j++) ason_string_free(&emps[i].skills.data[j]);
    ...
    printf("3. Map/Dict field:\n");
    {
        ...
        for (size_t i = 0; i < item.attrs.len; i++) {
            if (i) printf(",");
            printf("%s:%lld", item.attrs.data[i].key.data, (long long)item.attrs.data[i].val);
        }
        printf("}\n\n");
        ason_string_free(&item.name);
        for (size_t i = 0; i < item.attrs.len; i++) ason_string_free(&item.attrs.data[i].key);
    ...
    printf("4. Nested struct roundtrip:\n");
    {
        ...
        printf("   roundtrip OK\n\n");
        ason_buf_free(&buf);
        ason_string_free(&n.name); ason_string_free(&n.addr.city);
        ason_string_free(&n2.name); ason_string_free(&n2.addr.city);
        passed++;
    }
    ...
    printf("5. Escaped strings:\n");
    {
        ...
        ason_buf_free(&buf);
        ason_string_free(&note.text); ason_string_free(&note2.text);
        passed++;
    }
    ...
    printf("6. Float fields:\n");
    {
        ...
        ason_buf_free(&buf);
        ason_string_free(&m.label); ason_string_free(&m2.label);
        passed++;
    }
    ...
    printf("8. All types struct:\n");
    {
        ...
        for (int i = 0; i < 5; i++) ason_vec_i64_push(&all.vec_int, vi[i]);
        ...
        ason_vec_i64_push(&nv1, 1); ason_vec_i64_push(&nv1, 2);
        ason_vec_i64 nv2 = ason_vec_i64_new();
        ason_vec_i64_push(&nv2, 3); ason_vec_i64_push(&nv2, 4); ason_vec_i64_push(&nv2, 5);
        ...
        for (size_t i = 0; i < all.vec_str.len; i++) ason_string_free(&all.vec_str.data[i]);
        ason_vec_str_free(&all.vec_str);
        ason_vec_i64_free(&all.vec_int);
        for (size_t i = 0; i < all.nested_vec.len; i++) ason_vec_i64_free(&all.nested_vec.data[i]);
        ason_vec_vec_i64_free(&all.nested_vec);
        ason_string_free(&all2.s);
        for (size_t i = 0; i < all2.vec_str.len; i++) ason_string_free(&all2.vec_str.data[i]);
        ason_vec_str_free(&all2.vec_str);
        ason_vec_i64_free(&all2.vec_int);
        for (size_t i = 0; i < all2.nested_vec.len; i++) ason_vec_i64_free(&all2.nested_vec.data[i]);
    ...
    printf("9. Five-level nesting:\n");
    {
        ...
        City c1 = {ason_string_from("Ferriton"), 2000000, 350.5, ason_vec_District_new()};
        ...
        City c2 = {ason_string_from("Crabville"), 800000, 120.0, ason_vec_District_new()};
    ...
    printf("10. Seven-level nesting:\n");
    {
        ...
        Galaxy g = {ason_string_from("Milky Way"), 250.0, ason_vec_SolarSystem_new()};
        SolarSystem ss = {ason_string_from("Sol"), ason_string_from("G2V"), ason_vec_Planet_new()};

        Planet earth = {ason_string_from("Earth"), 6371.0, true, ason_vec_Continent_new()};
        Continent asia = {ason_string_from("Asia"), ason_vec_Nation_new()};
        Nation japan = {ason_string_from("Japan"), ason_vec_State_new()};
        State tokyo = {ason_string_from("Tokyo"), ason_string_from("Shinjuku"), 14000000};
        State osaka = {ason_string_from("Osaka"), ason_string_from("Osaka City"), 8800000};
        ...
        State beijing = {ason_string_from("Beijing"), ason_string_from("Beijing"), 21500000};
        ...
        State bavaria = {ason_string_from("Bavaria"), ason_string_from("Munich"), 13000000};
        State berlin = {ason_string_from("Berlin"), ason_string_from("Berlin"), 3600000};
        ...
        Planet mars = {ason_string_from("Mars"), 3389.5, false, ason_vec_Continent_new()};
    ...
    printf("11. Complex config struct:\n");
    {
        ...
        cfg.db = (DbConfig){ason_string_from("db.example.com"), 5432, 100, true, 3000.5};
        cfg.cache = (CacheConfig){true, 3600, 512};
        cfg.log = (LogConfig){ason_string_from("info"), {true, ason_string_from("/var/log/app.log")}, true};
        ...
        ason_map_ss_entry_t e1 = {ason_string_from("RUST_LOG"), ason_string_from("debug")};
        ason_map_ss_entry_t e2 = {ason_string_from("DATABASE_URL"), ason_string_from("postgres://localhost:5432/mydb")};
        ason_map_ss_entry_t e3 = {ason_string_from("SECRET_KEY"), ason_string_from("abc123!@#")};
        ...
        printf("   serialized (%zu bytes):\n   %.*s\n", buf.len, (int)(buf.len > 200 ? 200 : buf.len), buf.data);
        ...
        ason_buf_free(&buf);
        ason_string_free(&cfg.name); ason_string_free(&cfg.version);
        ason_string_free(&cfg.db.host); ason_string_free(&cfg.log.level);
        if (cfg.log.file.has_value) ason_string_free(&cfg.log.file.value);
        for (size_t i = 0; i < cfg.features.len; i++) ason_string_free(&cfg.features.data[i]);
        ason_vec_str_free(&cfg.features);
        for (size_t i = 0; i < cfg.env.len; i++) { ason_string_free(&cfg.env.data[i].key); ason_string_free(&cfg.env.data[i].val); }
        ason_map_ss_free(&cfg.env);
        ason_string_free(&cfg2.name); ason_string_free(&cfg2.version);
        ason_string_free(&cfg2.db.host); ason_string_free(&cfg2.log.level);
        if (cfg2.log.file.has_value) ason_string_free(&cfg2.log.file.value);
        for (size_t i = 0; i < cfg2.features.len; i++) ason_string_free(&cfg2.features.data[i]);
        ason_vec_str_free(&cfg2.features);
        for (size_t i = 0; i < cfg2.env.len; i++) { ason_string_free(&cfg2.env.data[i].key); ason_string_free(&cfg2.env.data[i].val); }
    ...
    printf("12. Edge cases:\n");
    {
        ...
        /* Special chars */
        Special sp = {ason_string_from("tabs\there, newlines\nhere, quotes\"and\\backslash")};
        ...
        ason_buf_free(&buf);
        ason_string_free(&sp.val); ason_string_free(&sp2.val);
        ...
        ason_buf_free(&buf);
        ason_string_free(&sp3.val); ason_string_free(&sp4.val);
        ...
        ason_buf_free(&buf);
        ason_string_free(&sp5.val); ason_string_free(&sp6.val);
    ...
    printf("13. Triple-nested arrays:\n");
    {
        ...
        ason_vec_i64_push(&r1, 1); ason_vec_i64_push(&r1, 2);
        ason_vec_i64 r2 = ason_vec_i64_new();
        ason_vec_i64_push(&r2, 3); ason_vec_i64_push(&r2, 4);
        ason_vec_i64 r3 = ason_vec_i64_new();
        ason_vec_i64_push(&r3, 5); ason_vec_i64_push(&r3, 6); ason_vec_i64_push(&r3, 7);
        ...
        ason_buf_free(&buf);
        for (size_t i = 0; i < m3.data.len; i++) ason_vec_i64_free(&m3.data.data[i]);
        ason_vec_vec_i64_free(&m3.data);
        for (size_t i = 0; i < m3b.data.len; i++) ason_vec_i64_free(&m3b.data.data[i]);
    ...
    printf("14. Comments:\n");
    {
        const char* input = "/* Top-level */ {id,name,dept:{title},skills,active}:/* inline */ (1,Alice,(HR),[rust],true)";
        Employee emp = {0};
        ason_err_t err = ason_load_Employee(input, strlen(input), &emp);
        assert(err == ASON_OK);
